...
class TrialsAdword extends AbstractChart
{
    /**
     * @param array $result
     * @param $year
    ...
    {
        $trials = Customer::select('date_created')
            ->where('date_created', '>=', "{$year}-{$month}-{$day} 00:00:00")
            ->where('date_created', '<=', "{$year}-{$month}-{$day} 23:59:59")
            ->where('ref_ads', 'like', '%gads%')
            ->get()
            ->groupBy(function ($t) {
                return (new \Carbon\Carbon($t->date_created))->format('H');
            })
            ->map(function ($c) {
                return $c->count();
            });
    ...
    protected static function daily(array $result, $year, $month)
    {
        ...
        $trials = Customer::select('date_created')
            ...
            ->get()
            ->groupBy(function ($t) {
                return (new \Carbon\Carbon($t->date_created))->format('d') - 1;
            })
            ->map(function ($c) {
                return $c->count();
            });
    ...
    protected static function yearly(array $result, $year)
    {
        $trials = Customer::select('date_created')
            ...
            ->get()
            ->groupBy(function ($t) {
                return (new \Carbon\Carbon($t->date_created))->format('m') -1;
            })
            ->map(function ($c) {
                return $c->count();
            });
